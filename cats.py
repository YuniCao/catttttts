...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    ok_paragraphs = [x for x in paragraphs if select(x)]
    if k <= len(ok_paragraphs)-1:
        return ok_paragraphs[k]
    else:
        return ''
    # END PROBLEM 1
...
def about(topic):
    ...
    def words(paragraph):
        ...
        for i in range(len(words_in_paragraph)):
            original = ""
            for c in words_in_paragraph[i]:
                 if c.isalpha():
                    original+=c.lower()
            words_in_paragraph[i]=original
        for i in topic:
            if i in words_in_paragraph:
                return True
        return False
    return words
...
def accuracy(typed, reference):
    ...
    typed_lst=typed.split()
    referenced_lst=reference.split()
    match=0
    total=0
    for i in range(len(typed_lst)):
        if i < len(referenced_lst) and typed_lst[i] == referenced_lst[i]:
            match +=1
        total+=1
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    d = {i : diff_function(typed_word, i, limit) for i in valid_words}
...
def feline_flips(start, goal, limit):
    ...
    diff = start[0] != goal[0] 
    return diff + feline_flips(start[1:], goal[1:], limit-diff)
...
def minimum_mewtations(start, goal, limit):
    ...
    def final_diff(start, goal, limit):
        if limit < 0:
            ...
            diff = start[0] != goal[0]
            add = 1 + final_diff(start, goal[1:], limit-1)
            remove = 1 + final_diff(start[1:], goal, limit-1)
            substitute = diff + final_diff(start[1:], goal[1:], limit - diff)
        return min(add,remove,substitute)
    return final_diff(start, goal, limit)
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(len(times_per_player[i])):
            if j != len(times_per_player[i]) - 1 :
                times0.append(times_per_player[i][j+1]-times_per_player[i][j])
        times.append(times0)
...
def fastest_words(game):
    ...
    for j in word_indices:
        fast_player=0
        fast_time=time(game,fast_player,j)

        for p in player_indices:
            player_time=time(game,p,j)
            if player_time<fast_time:
                fast_player=p
                fast_time=player_time
        result[fast_player].append(word_at(game,j))
    return result
